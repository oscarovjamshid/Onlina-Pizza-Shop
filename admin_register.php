...
if(isset($_POST['register'])){
   ...
   // sends credentials to DB phpadmin 
   $name = $_POST['name'];
   ...
   // connects to DB to select admin row by name
   $select_admin = $conn->prepare("SELECT * FROM `admin` WHERE name = ?");
   $select_admin->execute([$name]);

   // if inputted name found in DB, that name already registered
   if($select_admin->rowCount() > 0){
      ...
      if($pass != $cpass){
        $message[] = 'passwords not matched';
         ...
         // otherwise insert/register that name and password in DB
      else{
