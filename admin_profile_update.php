...
// if update is posted or sent to DB, then...
if(isset($_POST['update'])){


   // sends name credential to DB phpadmin 
   $name = $_POST['name'];
   $name = filter_var($name, FILTER_SANITIZE_STRING);

   // declare update profile var to find name in DB and update/change it to new one
   $update_profile_name = $conn->prepare("UPDATE `admin` SET name = ? WHERE id = ?");
   $update_profile_name->execute([$name, $admin_id]);

   // sends password credentials to DB phpadmin 
   ...
   // check old and new passwords match or not in form
   if($old_pass != $empty_pass)
   {
      ...
      elseif($new_pass != $confirm_pass)
      {
         ...
         {
            // if match then go for DB to find and update/change them 
